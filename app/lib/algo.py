...
def a_star_route_optimization(addresses, start_index):
    open_list = []
    heapq.heappush(
        open_list, (0, start_index, [], 0)
    )  # (f_score, current_node, path, distance_travelled)
    closed_list = set()
    optimal_route = []
    step_by_step_info = []
    ...
    while open_list:
        ...
        for neighbor_index, neighbor in enumerate(addresses):
            if neighbor_index != current_index and neighbor_index not in closed_list:
                ...
                step_info["options"].append(
                    {"address": neighbor["address"], "distance": g_score}
                )
        ...
        if valid_neighbors:
            ...
            heapq.heappush(
                open_list,
                (chosen_neighbor[0], chosen_neighbor[1], path, chosen_neighbor[0]),
            )
        else:
            break
